...
if __name__ == "__main__":
    ...
    sigma_psd_range = np.arange(0.01, 1.0, 0.01)
    custom_labels = [
        "nema",
        "droso",
        "mouse"
    ...
    # Determine the sigma value with the best overall performance
    mean_psnr_scores = np.mean(all_psnr_scores, axis=0)
    mean_ssim_scores = np.mean(all_ssim_scores, axis=0)
    
    best_sigma_overall_psnr = sigma_psd_range[np.argmax(mean_psnr_scores)]
    best_sigma_overall_ssim = sigma_psd_range[np.argmax(mean_ssim_scores)]

    print(f"Best overall sigma_psd value for PSNR: {best_sigma_overall_psnr}")
    print(f"Best overall sigma_psd value for SSIM: {best_sigma_overall_ssim}")
    ...
    plot_filename = 'psnr_ssim_comparison.png'
