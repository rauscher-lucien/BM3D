...
def save_filtered_stack(filtered_stack, input_path, sigma_psd, output_folder=""):
    if filtered_stack is None:
        return
    
    # Generate the output filename based on the input filename and sigma_psd value
    base_name = os.path.basename(input_path)
    name, ext = os.path.splitext(base_name)
    output_filename = f"{name}_filtered_bm3d_sigma_{sigma_psd}{ext}"
...
if __name__ == "__main__":
    ...
    output_path=r"C:\Users\rausc\Documents\EMBL\data\nema-results"
    sigma_psd = 0.09
    test_index = 60  # Example index to test a specific slice before processing the entire volume
    ...
    save_filtered_stack(filtered_stack, data_path, sigma_psd, output_path)
